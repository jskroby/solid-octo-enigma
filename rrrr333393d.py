#!/usr/bin/env python3
# hyperbot_monolith_cartman.py — Final Enhanced Edition

import os, sys, time, math, json, asyncio, logging, signal
...
SKIM_INTERVAL_SEC = float(os.getenv("SKIM_INTERVAL_SEC", 300))  # 5 min
SKIM_PERCENT = 0.8
LEDGER_FILE = "skim_ledger.json"
    ...
    async def skim_profits(self):
        if self.profit > 1.0:
            ...
            try:
                ...
                # Update Ledger
                self.ledger["total_btc_bought"] += btc_amount
                self.ledger["total_usd_skimmed"] += skim_amount
                self.ledger["last_skim_time"] = time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())
