#!/usr/bin/env python3
# hyperbot_studio_v2.py — Monolith: GA + Live Grid + Streamlit UI (v2)
...
# Prometheus
Gauge("cps", "¢/s").set_function(lambda: engine.calc_cps()*100)
Counter("profit", "USD profit")
Counter("trades", "trade count")
Thread(target=lambda: start_http_server(8001), daemon=True).start()

# Health endpoint
async def health(req): return web.Response(text="OK")
def start_http(): 
    app = web.Application(); app.router.add_get("/healthz", health)
...
with tab1:
    if st.button("▶️ Run GA Backtest"):
        ...
        fig = px.bar(df_results, x="Symbol", y="Fitness ¢/s", title="GA Fitness per Symbol")
        st.plotly_chart(fig, use_container_width=True)
        # plot evolution
        evo_fig = px.line(y=fitness_curve, labels={"y":"Best Fitness ¢/s","index":"Symbol Index"})
...
with tab2:
    if st.button("▶️ Launch Live Grid"):
        # initialize engine & grids
        engine = SteppedEngine(STEPS, LOCK_TIME, DEPOSIT, LEVERAGE)
        streams = [GridWorker(sym, engine, BASE_SP_PCT, GRID_LVLS, LOOP_INT,
                              FEE_TAKER, FEE_MAKER, SLIPP_PCT) for sym in SYMBOLS]
    ...
    while True:
        ...
        pnl = engine.total_profit
        cps_plot.metric("CPS (¢/s)", f"{cps:.3f}")
        pnl_text.metric("Total PnL (USD)", f"{pnl:.2f}")
        bal_text.write(f"**Balance:** {engine.get_balance():.4f} USDT")
        time.sleep(1)
...
def fetch_ws_closes(symbol, tf, limit):
    """Cacheable WS fetch of close series."""
    uri = "wss://fx-ws.gateio.ws/ws/spot"
    async def _inner():
        async with websockets.connect(uri) as ws:
            await ws.send(json.dumps({
                "time":int(time.time()), "channel":"spot.candlesticks",
                "event":"subscribe","payload":[symbol.replace("/","_"),str(int(tf[:-1])*60),str(limit)]
            }))
...
def run_ga(prices, pop, gens, cxpb, mutpb):
    """DEAP GA returns best, fitness, and history of best fitnesses."""
    ...
    tb.register("individual", make); tb.register("population", tools.initRepeat, list, make)
...
class SteppedEngine:
    def __init__(self, steps, lock_time, deposit, lev):
        self.steps = steps; self.lock_time = lock_time
        self.profit=0.0; self.t0=time.time()
        self.notional=(deposit*lev)/len(SYMBOLS)
        self.step=0; self.lock=None
    def calc_cps(self): return self.profit/max(time.time()-self.t0,1e-6)
    def record(self, gross):
        ...
        # tune
        factor=tgt/max(cps,1e-6); cap=DEPOSIT*LEVERAGE
        self.notional= max(1,min(self.notional*factor,cap))

class GridWorker:
    def __init__(self,sym,eng,base_sp,lvls,loop,ft, fm, sp):
        self.sym, self.e = sym, eng; self.spread=base_sp; self.lvls=lvls
        self.loop, self.ft, self.fm, self.spct = loop, ft, fm, sp
        self.ex=None
    async def run(self):
        self.ex = ccxt.gateio({
