...
profit_rate = 0.0
ga_iterations = 0
INITIAL_DEPOSIT = None  # will be set after fetching balance
start_time = None
...
def create_exchange():
    try:
        ...
        raise ImportError(
            "Error: ccxt module not found. Please install it with `pip install ccxt` and rerun."
        )
    return ccxt.gateio({
...
async def place_grid_orders(ex, symbol, levels, pos_size, market_type):
    ...
    for idx, price in enumerate(sorted_lv):
        ...
        try:
            ...
            orders.append(dict(id=order['id'], price=price, side=side, amount=amt, status='open', timestamp=time.time()))
...
async def monitor_grid(ex, symbol, orders, pos_size, market_type):
    ...
    grid_profit=0.0; trades=0; success=0
    volatility=fetch_volatility(symbol,ex) or 0.01
    profit_target=dynamic_profit_target(volatility,market_type)
    pulse=time.time(); active_grids[symbol]={'orders':orders,'profit':0,'trades':0,'success_rate':0.5}
    while symbol in active_grids:
        ...
        for o in list(orders):
            if o['status']=='open' and o['id'] not in open_ids:
                ...
                try:
                    ...
                    orders.append(dict(id=no['id'],price=new_price,side=side,amount=o['amount'],status='open',timestamp=time.time()))
                    grid_profit+=profit; trades+=1; success+=1
                    with thread_lock:
                        total_profit+=profit; total_trades+=1
        ...
        if grid_profit>0.2*pos_size:
            console.print(f"[blue]💤 {symbol} hit 20% ROI. Sleeping 6h.")
            for o in orders:
                if o['status']=='open':
                    try: await ex.cancel_order(o['id'],symbol)
                    except: pass
            del active_grids[symbol]
            await asyncio.sleep(21600); return
...
async def rotate_symbols(ex):
    while True:
        ...
        console.print("[cyan]🔄 Rotating Symbols...")
        syms=await get_best_symbols(ex,params.MAX_SYMBOLS)
        ps=(INITIAL_DEPOSIT-30.0)/params.MAX_SYMBOLS
        for s in list(active_grids):
            if s not in syms:
                console.print(f"[red]⚰️ Killing underperformer {s}")
                try: await ex.cancel_all_orders(s)
                except: pass
                del active_grids[s]
        for s in syms:
            if s not in active_grids:
                console.print(f"[green]🌱 Spawning new symbol {s}")
                await setup_symbol(ex,s,ps)

async def dynamic_ga_evolver():
    global ga_iterations
    tb=setup_ga(); pop=tb.population(n=10)
    while True:
        await asyncio.sleep(1800); ga_iterations+=1
        console.print(f"[bold magenta]🧬 Evolving GA Gen {ga_iterations}")
        fits=[evaluate_individual(i,[profit_rate]) for i in pop]
        for i,f in zip(pop,fits): i.fitness.values=f
        off=algorithms.varAnd(pop,tb,cxpb=0.5,mutpb=0.2)
        fits2=[evaluate_individual(i,[profit_rate]) for i in off]
        for i,f in zip(off,fits2): i.fitness.values=f
        pop=tools.selBest(pop+off,k=len(pop)); best=tools.selBest(pop,1)[0]
        apply_ga_params(best); console.print("[bold green]✅ GA Params updated!")

async def display_dashboard():
    while True:
        table=Table(title="🧬 Gigalithimonous Status")
        table.add_column("Sym"); table.add_column("Trades"); table.add_column("Profit"); table.add_column("Success %")
        with thread_lock:
            for s,d in active_grids.items():
                table.add_row(s,str(d['trades']),f"${d['profit']:.4f}",f"{d['success_rate']*100:.1f}%")
            elapsed=(time.time()-start_time)/60
            overview=f"⏱{elapsed:.1f}m | 💰${total_profit:.4f}"
            style="green" if profit_rate>=0.09 else "yellow" if profit_rate>=0.045 else "red"
        console.print(overview,style=style); console.print(table); await asyncio.sleep(5)

async def main():
    global INITIAL_DEPOSIT,start_time
    console.print("[bold cyan]🚀 Gigalithimonous GA^RL^GA^3 INITIATED")
    ex=create_exchange(); await ex.load_markets()
    INITIAL_DEPOSIT=(await ex.fetch_balance())['total']['USDT']; start_time=time.time()
    syms=await get_best_symbols(ex,params.MAX_SYMBOLS)
    ps=(INITIAL_DEPOSIT-30.0)/params.MAX_SYMBOLS
