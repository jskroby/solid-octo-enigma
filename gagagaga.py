...
def create_exchange():
    try:
        ...
        raise ImportError("Error: ccxt module not found. Please install it with `pip install ccxt` and rerun.")
    return ccxt.gateio({
...
async def setup_symbol(ex, symbol, pos_size):
    ...
    levels = [price_now * (1 + i * spacing / 100) for i in range(-4, 5)]
    orders = await place_grid_orders(ex, symbol, levels, pos_size, market_type)
    asyncio.create_task(monitor_grid(ex, symbol, orders, pos_size, market_type))
...
async def rotate_symbols(ex):
    while True:
        ...
        for s in list(active_grids):
            if s not in syms:
                console.print(f"⚰️ Killing underperformer {s}")
...
def setup_ga():
    ...
    toolbox.register("varAnd", algorithms.varAnd)
    return toolbox
...
async def dynamic_ga_evolver():
    ...
    while True:
        ...
        ga_iterations += 1
        fitnesses = [evaluate_individual(ind, [profit_rate]) for ind in population]
        for ind, fit in zip(population, fitnesses): ind.fitness.values = fit
        offspring = toolbox.varAnd(population, cxpb=0.5, mutpb=0.2)
        fits2 = [evaluate_individual(ind, [profit_rate]) for ind in offspring]
        for ind, fit in zip(offspring, fits2): ind.fitness.values = fit
        population = toolbox.select(population + offspring, k=len(population))
        best = toolbox.select(population, k=1)[0]
